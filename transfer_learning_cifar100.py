...
def main(args):
    ...
    #sh = SuccessiveHalving(cfgs, models, resource_budget=30, data_loader_train=data_loader_train, model_type=model_type, mixup_fn=mixup_fn, choices=choices, data_loader_val=data_loader_val, dataset_val=dataset_val)
    #weights_path, best_model_id = sh.run()
    best_model_id = 0
    weights_path = "OUTPUT/search/CIFAR100/best_model_0_weights.pth"
    ...
    if args.model_type == 'AUTOFORMER':
        ...
        if os.path.exists(weights_path):
            # Load the pre-trained weights, skipping the head layers
            pretrained_dict = torch.load(weights_path)

            # Remove keys corresponding to the head layers in your model
            for key in list(pretrained_dict.keys()):
                if 'head' in key:
                    del pretrained_dict[key]


            # Load the modified pre-trained weights into your model
    ...
    # Map CIFAR100 class names to their respective class indices
    class_to_idx = train_dataset.class_to_idx

    # Create data loaders for each category
    data_loaders = {}
    for super_category, classes in super_categories_to_classes.items():
        print(super_category)
        indices = [class_to_idx[c] for c in classes]
        train_dataset_filtered = torch.utils.data.Subset(train_dataset,
                                                         [i for i, (_, target) in enumerate(train_dataset) if
                                                          target in indices])
        test_dataset_filtered = torch.utils.data.Subset(test_dataset,
                                                        [i for i, (_, target) in enumerate(test_dataset) if
                                                         target in indices])

        train_loader = torch.utils.data.DataLoader(train_dataset_filtered, batch_size=32, shuffle=True, num_workers=2)
        test_loader = torch.utils.data.DataLoader(test_dataset_filtered, batch_size=32, shuffle=False, num_workers=2)

        data_loaders[super_category] = (train_loader, test_loader)

        if len(train_loader.dataset) == 0:
            print("Train Loader is empty.")
        else:
            print(f"Number of samples in the train loader: {len(train_loader.dataset)}")

        if len(test_loader.dataset) == 0:
            print("Test Loader is empty.")
        else:
            print(f"Number of samples in the test loader: {len(test_loader.dataset)}")

    accuracies = np.zeros((20,20))
    train_accuracies = np.zeros((20,20))
    # Create a scatter plot
    file_path = 'poc\\accuracies_transfer_learning.txt'

    # Read the matrix from the file
    try:
        with open(file_path, 'r') as file:
            lines = file.readlines()
            accuracies_old = [list(map(float, line.strip().split())) for line in lines]
        accuracies_old = np.array(accuracies_old)

    except IOError:
        print(f"Could not read the file at {file_path}. Please check the file path and try again.")

    for i, (self_super_category, classes) in enumerate(super_categories_to_classes.items()):

        data_loader_train,data_loader_val = data_loaders[self_super_category]

        for j, (super_category, classes) in enumerate(super_categories_to_classes.items()):

            if (i == j):
                continue

            if(accuracies_old[i,j] > 0):
                accuracies[i,j] = accuracies_old[i,j]
                continue
            max_accuracy = 0.0
            if args.model_type == 'AUTOFORMER':
                model_type = args.model_type
                ...
                retrain_config = {'layer_num': cfg.RETRAIN.DEPTH, 'base_dim': cfg.RETRAIN.BASE_DIM,
                                  'num_heads': cfg.RETRAIN.NUM_HEADS, 'mlp_ratio': cfg.RETRAIN.MLP_RATIO,
                                  'patch_size': cfg.RETRAIN.PATCH_SIZE}
                model = pit(retrain_config, pretrain=False)
                choices = {'num_heads': cfg.SEARCH_SPACE.NUM_HEADS, 'mlp_ratio': cfg.SEARCH_SPACE.MLP_RATIO,
                           'base_dim': cfg.SEARCH_SPACE.BASE_DIM, 'depth': cfg.SEARCH_SPACE.DEPTH,
                           'patch_size': cfg.SEARCH_SPACE.PATCH_SIZE}
            ...
            weights_path = 'poc//{}_weights.pth'.format(super_category)
            pretrained_dict = torch.load(weights_path)
            # Remove keys corresponding to the head layers in your model
            for key in list(pretrained_dict.keys()):
                if 'head' in key:
                    del pretrained_dict[key]
            # Load the modified pre-trained weights into your model
            print("Loading weights {}".format(super_category))
            model.load_state_dict(pretrained_dict, strict=False)
            max_accuracy = 0.0
            for epoch in range(args.start_epoch, epochs):
                ...
                train_stats = evaluate(data_loader_train, model_type, model, device, amp=args.amp, choices=choices,
                                      mode=args.mode, retrain_config=retrain_config)
                print(f"Accuracy of the network on {self_super_category} - {super_category} train images: {train_stats['acc1']:.1f}%")
                ...
                print(f"Accuracy of the network on {self_super_category} - {super_category} test images: {test_stats['acc1']:.1f}%")
                if (test_stats["acc1"] > max_accuracy):
                        accuracies[i][j] = test_stats["acc1"]
                        train_accuracies[i][j] = train_stats["acc1"]
                np.savetxt('poc//accuracies_transfer_learning.txt', accuracies)
                np.savetxt('poc//train_accuracies_transfer_learning.txt', train_accuracies)
