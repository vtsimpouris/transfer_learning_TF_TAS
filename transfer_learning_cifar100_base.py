...
def main(args):
    ...
    file_path = 'poc\\accuracies.txt'

    # Read the matrix from the file
    try:
        with open(file_path, 'r') as file:
            lines = file.readlines()
            prev_accuracies = [list(map(float, line.strip().split())) for line in lines]
        prev_accuracies = np.array(prev_accuracies)
    except IOError:
        print(f"Could not read the file at {file_path}. Please check the file path and try again.")

    for i, (super_category, classes) in enumerate(super_categories_to_classes.items()):
        if (prev_accuracies[i] > 0):
            continue
        data_loader_train,data_loader_val = data_loaders[super_category]
        max_accuracy = 0.0
        ...
        if args.mode == 'retrain' and "RETRAIN" in cfg and model_type == 'AUTOFORMER':
            ...
            retrain_config = {'layer_num': cfg['RETRAIN']['DEPTH'],
                              ...
                              'num_heads': cfg['RETRAIN']['NUM_HEADS'], 'mlp_ratio': cfg['RETRAIN']['MLP_RATIO']}
        if args.eval:
            test_stats = evaluate(data_loader_val, model_type, model, device, mode=args.mode,
        ...
        for epoch in range(args.start_epoch, epochs):
            ...
            print(f"Accuracy of the network on {super_category} train images: {train_stats['acc1']:.1f}%")
            ...
            print(f"Accuracy of the network on {super_category} test images: {test_stats['acc1']:.1f}%")
            if (test_stats["acc1"] > max_accuracy):
                if epoch < 26:
                    accuracies[i] = test_stats["acc1"]
                np.savetxt('poc//accuracies.txt', accuracies)
